...
ARTICLE_PATH = Path("data/literary_journal_articles_opencitations.xlsx")
METRICS_PATH = Path("data/literary_journal_articles_opencitations_metrics.xlsx")

OUT_DIR = Path("data/figures")
OUT_DIR.mkdir(parents=True, exist_ok=True)

OUTPUT_HTML = OUT_DIR / "05_citation_regime_map_interactive_explained_search.html"
OUTPUT_METRICS = Path("data/literary_journal_articles_opencitations_metrics_with_regimes.xlsx")
...
def load_articles(article_path):
    ...
    required_cols = ["article_omid", "venue_name", "citedby_count"]
    ...
    articles = raw[required_cols].copy()
    articles = articles.dropna(subset=["article_omid", "venue_name"])

    articles["citedby_count"] = (
        pd.to_numeric(articles["citedby_count"], errors="coerce")
        .fillna(0)
        .clip(lower=0)
    )
    ...
    articles = (
        articles
        .groupby(["article_omid", "venue_name"], as_index=False)["citedby_count"]
        .max()
    )
...
def get_or_compute_metrics(metrics_path, article_path):
    ...
    if metrics_path.exists():
        ...
        print(
            ...
            "Recomputing metrics from the article-level file."
        )
...
def build_regime_figure(metrics, x_median, y_median):
    ...
    fig.update_layout(
        ...
        title=dict(
            text=(
                "Citation regimes of journals in OpenCitations"
                "<br><sup>"
...
<head>
<meta charset="utf-8">
<title>Citation regimes of journals in OpenCitations</title>
...
<h1>Citation regimes of journals in OpenCitations</h1>
...
The categories are relative to this dataset; they are not universal journal types.
</div>
...
The chart describes citation visibility captured in OpenCitations, not journal quality.
The results may be affected by metadata coverage, identifiers, language, publication age, disciplinary citation practices,
and the completeness of citation links available in the source data.
</div>
