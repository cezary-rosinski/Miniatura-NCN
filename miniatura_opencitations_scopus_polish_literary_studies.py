    ...
    for attempt in range(max_retries):
        try:
            ...
            if response_json:
                ...
                omids = [elem.replace('omid:', '') for elem in [ele for sub in [[el for el in e.split(' ')] for e in ids] for ele in sub] if 'omid:' in elem]
...
a = df.loc[0, 'OMID for ISSN']
b = df.loc[0, 'OMID for e-ISSN']
...
df.to_excel('data/literary_journals_with_omid.xlsx', index=False)

# 3 Count of citations for a venue based on issn
df = pd.read_excel('data/literary_journals_with_omid.xlsx')
...
df.to_excel('data/literary_journals_with_omid_citations_counted.xlsx', index=False)
...
# 1. Wczytanie danych
# =========================
file_path = "data/literary_journals_with_omid_citations_counted.xlsx"
...
file_path = "data/literary_journals_with_omid_citations_counted.xlsx"
df = pd.read_excel(file_path)
df['OMID'] = df['OMID'].apply(
    lambda x: ast.literal_eval(x) if isinstance(x, str) else x
)
omids = set([el for sub in [e for e in df['OMID'].to_list() if isinstance(e, list)] for el in sub])
...
df = pd.read_excel("data/literary_journals_with_omid_citations_counted.xlsx")
...
df.to_excel('data/literary_journals_opencitation_final.xlsx', index=False)
