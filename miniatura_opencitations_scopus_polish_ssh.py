...
df = pd.read_excel('data/scopus_journals_full.xlsx')
...
df.to_excel('data/scopus_journals_full_with_omid.xlsx', index=False)

# 3 Count of citations for a venue based on issn
df = pd.read_excel('data/scopus_journals_full_with_omid.xlsx')
...
agg_dict = {
    col: "first" for col in df.columns if col != "Dyscyplina"
}

agg_dict["Dyscyplina"] = lambda x: "|".join(
    sorted(
        pd.Series(x)
        .dropna()
        .astype(str)
...
df_reduced = (
    df.groupby("Tytuł", dropna=False, as_index=False)
      .agg(agg_dict)
)

df_reduced.to_excel('data/scopus_journals_full_with_omid_citations_counted.xlsx', index=False)
...
# 1. Wczytanie danych
# =========================
file_path = "data/scopus_journals_full_with_omid_citations_counted.xlsx"
